...
func (r *SubjectControlRepository) Create(ctx context.Context, control SubjectControl) (SubjectControl, error) {
	...
	err := r.pool.QueryRow(
		...
		`INSERT INTO subject_controls (subject_id, type_id, semester_num, semester_id)
		 VALUES ($1, $2, $3, $4)
		 RETURNING control_id, subject_id, type_id, semester_num, semester_id`,
		control.SubjectID,
		control.TypeID,
		control.CurriculumSemesterNum,
		control.SemesterID,
	).Scan(&out.ID, &out.SubjectID, &out.TypeID, &out.CurriculumSemesterNum, &out.SemesterID)
	if err != nil {
...
func (r *SubjectControlRepository) GetByID(ctx context.Context, id int32) (SubjectControl, bool, error) {
	...
	err := r.pool.QueryRow(
		ctx,
		`SELECT control_id, subject_id, type_id, semester_num, semester_id
...
func (r *SubjectControlRepository) ListBySubjectID(ctx context.Context, subjectID int32) ([]SubjectControl, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.SubjectID, &item.TypeID, &item.CurriculumSemesterNum, &item.SemesterID); err != nil {
