...
Перед показом или рабочим запуском прочитайте [инструкцию подготовки окружения](backend/docs/LAUNCH_READINESS.md): JWT-ключ обязателен, демонстрационные аккаунты и общие коды регистрации отключены по умолчанию.
...
- Посещаемость: преподаватель открывает сессию (QR/инвайт-токен с ограниченным сроком жизни), студент подтверждает по токену; веб- и Android-клиенты передают ID устройства и геолокацию для антифрода, а без разрешения геолокации отметка не принимается
...
- `JWT_SECRET` (обязательно): случайный ключ подписи JWT не короче 32 символов; значения-заглушки отклоняются при запуске
- `ALLOW_DEMO_ACCOUNTS` (по умолчанию `false`): разрешить встроенные демонстрационные аккаунты на изолированном стенде
- `ALLOW_LEGACY_ROLE_REGISTRATION` (по умолчанию `false`): явно включить устаревшую регистрацию по общим кодам ролей
...
- `ROLE_HASH_TEACHER` / `ROLE_HASH_STUDENT` (нужны для legacy hash-регистрации): общие секреты для самостоятельной регистрации по роли  
  По умолчанию пустые; используются только при `ALLOW_LEGACY_ROLE_REGISTRATION=true`
...
- миграции создают тестовые профили; вход в них разрешается только при `ALLOW_DEMO_ACCOUNTS=true` на изолированном стенде. Актуальные пароли демонстрационных аккаунтов хранятся локально в игнорируемом Git-файле `output/presentation-credentials.json`; пароли из старых инструкций использовать нельзя.
...
| `POST /register/by-invite` | без токена | регистрация по инвайт-коду, роль определяется записью в БД; переданное согласие принимается только для текущей версии и сохраняется атомарно |
...
Поле `agreement` можно не передавать старым клиентам. Если оно передано, регистрация отклоняется при отсутствии принятия или при версии, отличной от `2026-09-01`.
