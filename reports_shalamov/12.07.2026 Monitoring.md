...
* **Оркестрация (Kubernetes):** Проанализирована необходимость развертывания сервиса в K8s, подготовлены некоторые манифесты, но внедрение отложено как избыточное на текущем этапе проверки гипотез(поможет ли horizontal scaling увеличить rps на первом логине), а сама задача перенесена в наш бэклог
